	...
	int analyze_module(scanner_search* scanner, module_search* module_data)
	{
		...
		for (ZyanUSize offset = 0; offset < module_data->text_size;)
		{
			...
			if (ZYAN_SUCCESS(status))
			{
				...
				{
					std::string symbol_name = memory::get_symbol_name(scanner->process_handle, runtime_address, module_data->remote_module_base);
					...
					case ZYDIS_MNEMONIC_JMP:
					{
						...
						if (ZYAN_SUCCESS(absolute_jmp_status) && absolute_jmp_instruction->mnemonic == ZYDIS_MNEMONIC_JMP)
						{
							...
							printf("[2] [%p] [%s!%s] [jmp %s]\n", (void*)runtime_address, module_data->module_name.c_str(), symbol_name.c_str(), absolute_jmp_destination_formatted.c_str());
						}
						else
						{
							printf("[1] [%p] [%s!%s] [jmp %s]\n", (void*)runtime_address, module_data->module_name.c_str(), symbol_name.c_str(), jmp_destination_formatted.c_str());
						}
					...
					case ZYDIS_MNEMONIC_INT3:
					{
						...
						printf("[0] [%p] [%s!%s] [%s]\n", (void*)runtime_address, module_data->module_name.c_str(), symbol_name.c_str(), instruction_buffer.c_str());
						break;
					}
